...
const DiscussionList = ({ onDiscussionClick }) => {
  ...
  const filteredDiscussions = discussions.filter((d) => {
    ...
    const matchesCategory =
      !selectedCategory || d.category.name === selectedCategory;
        ...
        <div className="relative w-full md:w-auto">
          ...
          {isDropdownOpen && (
            <div className="absolute top-full left-0 mt-2 w-full md:w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
              {['', 'Umum', 'Teknologi', 'Game', 'Berita'].map((cat, idx) => (
                <button
                  key={idx}
                  onClick={() => {
      ...
      <div className="overflow-x-auto overflow-y-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/5">Judul Diskusi</th>
